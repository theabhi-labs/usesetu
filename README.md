...
- **Prompt 3 — Category Builder (No-Code)**: Unlimited nested categories, Cloudflare R2 banner upload, SEO fields, sort order, featured/homepage toggles, public tree endpoint, drag-drop reorder endpoint.
...
├── middlewares/    # auth, rbac, security, validation, upload
├── services/       # engines: workflow, request, billing, queueEngine, appointmentEngine,
│                   # automationEngine, analyticsEngine, notificationDispatch, eventBus, email, r2
...
npm install
cp .env.example .env
# edit .env with your real MongoDB URI, JWT secrets, Brevo key, Cloudflare R2 keys
...
| POST / GET / DELETE | `/media` | Admin | Upload to Cloudflare R2 + searchable catalog |
